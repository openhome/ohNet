        ...
        if ((ptr[i] == 0xff) && ((ptr[i+1] & 0xe0) == 0xe0)) {
            ...
            try {
                ...
                for (TUint syncsFound = 0; syncsFound < kSyncsReq; syncsFound++) {
                    ...
                    if ((last_layer != layer) || (last_mode != mode)) {
...
CodecMp3::CodecMp3() 
    : iHeader(NULL)
    , iHeaderBytes(0)
    , iInput(kReadReqBytes + MAD_BUFFER_GUARD)
{
...
TBool CodecMp3::Recognise(const Brx& aMsg)
{
    ASSERT(iHeaderBytes == 0);

    if (aMsg.Bytes() > 4) {
        TUint syncFrameOffsetBytes = 0;
        if (Mp3Header::Exists(aMsg, syncFrameOffsetBytes)) {
...
void CodecMp3::StreamInitialise()
{
    ...
    iTrackLengthJiffies = (iHeader->SamplesTotal() * Jiffies::kJiffiesPerSecond) / iHeader->SampleRate();
