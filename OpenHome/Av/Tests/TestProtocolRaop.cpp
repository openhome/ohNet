...
DummyFiller::DummyFiller(Environment& aEnv, Net::DvStack& aDvStack, const TChar* aHostName, const TChar* aFriendlyName, const Brx& aMacAddr, ISupply& aSupply, IFlushIdProvider& aFlushIdProvider, Av::IInfoAggregator& aInfoAggregator)
    : Thread("SPHt")
    ...
    , iNextStreamId(kInvalidPipelineId+1)
{
    ...
    iProtocolManager->Add(new ProtocolRaop(aEnv, *iRaopDiscovery, iServerManager, audioId, controlId));
...
TestProtocolRaop::TestProtocolRaop(Environment& aEnv, Net::DvStack& aDvStack, const TChar* aHostName, const TChar* aFriendlyName, const Brx& aMacAddr, const Brx& aUrl, const Brx& aSenderUdn, TUint aSenderChannel)
    : iUrl(aUrl)
    , iStreamId(0)
{
...
int CDECL main(int aArgc, char* aArgv[])
{
    ...
    OptionString optionMac("-m", "--mac", Brn("000000000001"), "[mac] mac address for advertising via raop");
    parser.AddOption(&optionMac);
    ...
    TestProtocolRaop* tph = new TestProtocolRaop(lib->Env(), *dvStack, optionHost.CString(), optionName.CString(), optionMac.Value(), Brn("raop://dummyuri"), optionUdn.Value(), optionChannel.Value());
