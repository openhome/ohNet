...
DeviceList::DeviceList(Stack& aStack)
    : iStack(aStack)
    , iLock("DLMX")
    , iStopped(false)
    , iSem("DLSM", 0)
...
void DeviceList::Poll()
{
    ...
    for (TUint i=0; i<count; i++) {
        ...
        TUint startTime = Os::TimeInMs(iStack.OsCtx());
        while(true) {
            iSem.Clear();
            connMgr->Subscribe();
            try {
            ...
            connMgr->Unsubscribe();
            if (Os::TimeInMs(iStack.OsCtx()) - startTime > kDevicePollMs) {
                break;
            }
            gSubscriptionCount++;
        }
...
void TestSubscription(CpStack& aCpStack)
{
    ...
    DeviceList* deviceList = new DeviceList(stack);
    ...
    Print("\n%u subscriptions on %u devices (avg %u) in %u seconds\n",
                        gSubscriptionCount, count, (count==0? 0 : gSubscriptionCount/count), (Os::TimeInMs(stack.OsCtx())-startTime+500)/1000);
