...
fail:
    LogMsg("DNSServiceBrowse(\"%s\", \"%s\") failed: %s (%ld)", regtype, domain, errormsg, err);
    if (x != NULL) { DNSServiceRefDeallocate((DNSServiceRef)x); }
    return(err);
}
...
#if 0 // mDNS_DirectOP_GetAddrInfo struct is not defined in mDNSResponder v333.10 codebase
...
#endif // mDNS_DirectOP_GetAddrInfo disable
//*************************************************************************************************************
