...
int ogg2_page_version(ogg_page *og){
  oggbyte_buffer ob;
  oggbyte_init(&ob,og->header);
  return oggbyte_read1(&ob,4);
}

int ogg2_page_continued(ogg_page *og){
  oggbyte_buffer ob;
  oggbyte_init(&ob,og->header);
  return oggbyte_read1(&ob,5)&0x01;
}

int ogg2_page_bos(ogg_page *og){
  oggbyte_buffer ob;
  oggbyte_init(&ob,og->header);
  return oggbyte_read1(&ob,5)&0x02;
}

int ogg2_page_eos(ogg_page *og){
  oggbyte_buffer ob;
  oggbyte_init(&ob,og->header);
  return oggbyte_read1(&ob,5)&0x04;
}

ogg_int64_t ogg2_page_granulepos(ogg_page *og){
  oggbyte_buffer ob;
  oggbyte_init(&ob,og->header);
  return oggbyte_read8(&ob,6);
}

ogg_uint32_t ogg2_page_serialno(ogg_page *og){
  oggbyte_buffer ob;
  oggbyte_init(&ob,og->header);
  return oggbyte_read4(&ob,14);
}
 
ogg_uint32_t ogg2_page_pageno(ogg_page *og){
  oggbyte_buffer ob;
  oggbyte_init(&ob,og->header);
...
int ogg2_page_packets(ogg_page *og){
  int i;
  int n;
  int count=0;
  oggbyte_buffer ob;
...
ogg_sync_state *ogg2_sync_create(void){
...
int ogg2_sync_destroy(ogg_sync_state *oy){
  if(oy){
    ogg2_sync_reset(oy);
...
unsigned char *ogg2_sync_bufferin(ogg_sync_state *oy, long bytes){
...
int ogg2_sync_wrote(ogg_sync_state *oy, long bytes){
...
long ogg2_sync_pageseek(ogg_sync_state *oy,ogg_page *og){
  oggbyte_buffer page;
  long           bytes,ret=0;
...
int ogg2_sync_pageout(ogg_sync_state *oy, ogg_page *og){
...
int ogg2_sync_reset(ogg_sync_state *oy){
...
ogg_stream_state *ogg2_stream_create(int serialno){
...
int ogg2_stream_destroy(ogg_stream_state *os){
  if(os){
...
int ogg2_stream_pagein(ogg_stream_state *os, ogg_page *og){
...
int ogg2_stream_reset(ogg_stream_state *os){
...
int ogg2_stream_reset_serialno(ogg_stream_state *os,int serialno){
...
int ogg2_stream_packetout(ogg_stream_state *os,ogg_packet *op){
  return _packetout(os,op,1);
}

int ogg2_stream_packetpeek(ogg_stream_state *os,ogg_packet *op){
  return _packetout(os,op,0);
}

int ogg2_packet_release(ogg_packet *op) {
  if(op){
...
int ogg2_page_release(ogg_page *og) {
  if(og){
...
void ogg2_page_dup(ogg_page *dup,ogg_page *orig){
