/*___________________________________________________________________________
 |                                                                           |
 |   Constants and Globals                                                   |
 |                                                                           |
 | $Id: basic_op.h,v 1.8 2005-02-20 12:07:26 ehr Exp $
 |___________________________________________________________________________|
*/

#ifndef __BASIC_OP_H
#define __BASIC_OP_H


#include "typedef.h"

extern Flag Overflow;
extern Flag Carry;

#define MAX_32 (Word32)0x7fffffffL
#define MIN_32 (Word32)0x80000000L

#define MAX_16 (Word16)0x7fff
#define MIN_16 (Word16)0x8000

/*___________________________________________________________________________
 |                                                                           |
 |   Prototypes for basic arithmetic operators                               |
 |___________________________________________________________________________|
*/

//#define MACRO_OPTIMIZED_L_abs
//#define MACRO_OPTIMIZED_L_negate
//#define MACRO_OPTIMIZED_L_sub
//#define MACRO_OPTIMIZED_L_add
//#define MACRO_OPTIMIZED_L_mult
//#define MACRO_OPTIMIZED_mult
//#define MACRO_OPTIMIZED_extract_h
//#define MACRO_OPTIMIZED_extract_l

Word16 add (Word16 var1, Word16 var2);    /* Short add,           1   */
Word16 sub (Word16 var1, Word16 var2);    /* Short sub,           1   */
Word16 abs_s (Word16 var1);               /* Short abs,           1   */
Word16 shl (Word16 var1, Word16 var2);    /* Short shift left,    1   */
Word16 shr (Word16 var1, Word16 var2);    /* Short shift right,   1   */
Word16 negate (Word16 var1);              /* Short negate,        1   */
Word16 etsiopround (Word32 L_var1);             /* Round,               1   */
Word32 L_mac (Word32 L_var3, Word16 var1, Word16 var2);   /* Mac,  1  */
Word32 L_msu (Word32 L_var3, Word16 var1, Word16 var2);   /* Msu,  1  */
Word32 L_macNs (Word32 L_var3, Word16 var1, Word16 var2); /* Mac without
                                                             sat, 1   */
Word32 L_msuNs (Word32 L_var3, Word16 var1, Word16 var2); /* Msu without
                                                             sat, 1   */
Word32 L_add_c (Word32 L_var1, Word32 L_var2);  /* Long add with c, 2 */
Word32 L_sub_c (Word32 L_var1, Word32 L_var2);  /* Long sub with c, 2 */
Word16 mult_r (Word16 var1, Word16 var2);       /* Mult with round, 2 */
Word16 shr_r (Word16 var1, Word16 var2);        /* Shift right with
                                                   round, 2           */
Word16 mac_r (Word32 L_var3, Word16 var1, Word16 var2); /* Mac with
                                                           rounding,2 */
Word16 msu_r (Word32 L_var3, Word16 var1, Word16 var2); /* Msu with
                                                           rounding,2 */
Word32 L_deposit_h (Word16 var1);        /* 16 bit var1 -> MSB,     2 */
Word32 L_deposit_l (Word16 var1);        /* 16 bit var1 -> LSB,     2 */

Word32 L_shr_r (Word32 L_var1, Word16 var2); /* Long shift right with
                                                round,  3             */
Word32 L_sat (Word32 L_var1);            /* Long saturation,       4  */
Word16 norm_s (Word16 var1);             /* Short norm,           15  */
Word16 div_s (Word16 var1, Word16 var2); /* Short division,       18  */
Word16 norm_l (Word32 L_var1);           /* Long norm,            30  */   
Word32 L_shl (Word32 L_var1, Word16 var2);      /* Long shift left, 2 */
Word32 L_shr (Word32 L_var1, Word16 var2);      /* Long shift right, 2*/

#ifdef MACRO_OPTIMIZED_L_abs
#define L_abs(a) (a<0?((a== MIN_32)?MAX_32:-a):a)
#else
Word32 L_abs (Word32 L_var1);            /* Long abs,              3  */
#endif

#ifdef MACRO_OPTIMIZED_L_negate
#define L_negate(a) (-a)
#else
Word32 L_negate (Word32 L_var1);                /* Long negate,     2 */
#endif

#ifdef MACRO_OPTIMIZED_L_sub
#define L_sub(a,b) (a-b)
#else
Word32 L_sub (Word32 L_var1, Word32 L_var2);    /* Long sub,        2 */
#endif

#ifdef MACRO_OPTIMIZED_L_add
#define L_add(a,b) (a+b)
#else
Word32 L_add (Word32 L_var1, Word32 L_var2);    /* Long add,        2 */
#endif

#ifdef MACRO_OPTIMIZED_L_mult
#define L_mult(a,b) ((Word32)a * (Word32)b * 2)
#else
Word32 L_mult (Word16 var1, Word16 var2); /* Long mult,           1   */
#endif

#ifdef MACRO_OPTIMIZED_mult
#define mult(a,b) ((Word16)(((Word32)a * (Word32)b) >> 15))
#else
Word16 mult (Word16 var1, Word16 var2);   /* Short mult,          1   */
#endif

#ifdef MACRO_OPTIMIZED_extract_h
#define extract_h(a) ((Word16)(a >> 16))
#else
Word16 extract_h (Word32 L_var1);         /* Extract high,        1   */
#endif

#ifdef MACRO_OPTIMIZED_extract_l
#define extract_l(a) ((Word16)a)
#else
Word16 extract_l (Word32 L_var1);         /* Extract low,         1   */
#endif


#endif
