...
/* read a little more data from the file/pipe into the ogg2_sync framer */
static long _get_data(OggVorbis_File *vf){
  errno=0;
  if(vf->datasource){
    unsigned char *buffer=ogg2_sync_bufferin(vf->oy,CHUNKSIZE);
    long bytes=(vf->callbacks.read_func)(buffer,1,CHUNKSIZE,vf->datasource);
    if(bytes>0)ogg2_sync_wrote(vf->oy,bytes);
    if(bytes==0 && errno)return(-1);
    vf->callbacks.print_func(vf->datasource, "_get_data success\n");
    return(bytes);
  }else
    return(0);
}
...
static ogg_int64_t _get_next_page(OggVorbis_File *vf,ogg_page *og,
  ...
  vf->callbacks.print_func(vf->datasource, "_get_next_page\n");
  while(1){
    long more;

    if(boundary>0 && vf->offset>=boundary)return(OV_FALSE);
    more=ogg2_sync_pageseek(vf->oy,og);
	...
	if(!boundary){
		  vf->callbacks.print_func(vf->datasource, "_get_next_page exit OV_FALSE\n");
		  return(OV_FALSE);
	}
	{
	  long ret=_get_data(vf);
	  if(ret==0) {
		  vf->callbacks.print_func(vf->datasource, "_get_next_page exit OV_EOF\n");
		  return(OV_EOF);
	  }
	  if(ret<0) {
		  vf->callbacks.print_func(vf->datasource, "_get_next_page exit OV_EREAD\n");
		  return(OV_EREAD);
	  }
	}
      }else{
	...
	vf->offset+=more;
	  vf->callbacks.print_func(vf->datasource, "_get_next_page exit\n");
	return(ret);
...
static ogg_int64_t _get_prev_page(OggVorbis_File *vf,ogg_page *og){
  ...
  vf->callbacks.print_func(vf->datasource, "_get_prev_page\n");
...
static ogg_int64_t _get_prev_page_serial(OggVorbis_File *vf,
  ...
  vf->callbacks.print_func(vf->datasource, "_get_prev_page_serial\n");
  ...
  while(offset==-1){
    ...
    while(vf->offset<end){
      ...
      if(ret<0){
        ...
        ret_serialno=ogg2_page_serialno(&og);
        ret_gran=ogg2_page_granulepos(&og);
        offset=ret;
        ogg2_page_release(&og);
...
/* uses the local ogg2_stream storage in vf; this is important for
...
static int _fetch_headers(OggVorbis_File *vf,
  ...
  int i,ret;
  int allbos=0;
  vf->callbacks.print_func(vf->datasource, "_fetch_headers\n");
  ...
  while(ogg2_page_bos(og_ptr)){
    ...
    if(vf->ready_state<STREAMSET){
      ...
      ogg2_stream_reset_serialno(vf->os,ogg2_page_serialno(og_ptr));
      ogg2_stream_pagein(vf->os,og_ptr);

      if(ogg2_stream_packetout(vf->os,&op) > 0 &&
    ...
    /* get next page */
    {
      ...
      if(vf->ready_state==STREAMSET &&
         vf->os->serialno == ogg2_page_serialno(og_ptr)){
        ogg2_stream_pagein(vf->os,og_ptr);
        break;
      }
    }
  ...
  while(1){
    ...
    while(i<2){ /* get a page loop */
      ...
      while(i<2){ /* get a packet loop */
        ...
        int result=ogg2_stream_packetout(vf->os,&op);
        if(result==0)break;
        if(result<0){	// OGG_HOLE or OGG_SPAN
          ret=OV_EBADHEADER;
          goto bail_header;
        }
      ...
      while(i<2){
        ...
        if(vf->os->serialno == ogg2_page_serialno(og_ptr)){
          ogg2_stream_pagein(vf->os,og_ptr);
          break;
        }
    ...
    ogg2_page_release(&og);
    vf->callbacks.print_func(vf->datasource, "\n_fetch_headers exit\n");
...
static ogg_int64_t _initial_pcmoffset(OggVorbis_File *vf, vorbis_info *vi){
  ...
  vf->callbacks.print_func(vf->datasource, "_initial_pcmoffset\n");
  ...
  while(1){
    ...
    if(ogg2_page_bos(&og)) break;
    if(ogg2_page_serialno(&og)!=serialno) continue;
    pos=ogg2_page_granulepos(&og);

    /* count blocksizes of all frames in the page */
    ogg2_stream_pagein(vf->os,&og);
    while((result=ogg2_stream_packetout(vf->os,&op))){
...
static int _bisect_forward_serialno(OggVorbis_File *vf,
  ...
  int serialno = vf->os->serialno;
  vf->callbacks.print_func(vf->datasource, "_bisect_forward_serialno\n");
  ...
  */
  ...
  if(_lookup_serialno(endserial,currentno_list,currentnos)){
    ...
    vorbis_comment vc;
    vorbis_info vi;
    vi.codec_setup = 0;	//ensure that setup info is correctly initialised/freed
...
static int _fetch_and_process_packet(OggVorbis_File *vf,
  ...
  ogg_packet op={0,0,0,0,0,0};
  int ret=0;
  vf->callbacks.print_func(vf->datasource, "_fetch_and_process_packet\n");
  ...
  while(1){
	...
	/* bitrate tracking; add the header's bytes here, the body bytes
	   are done by packet above */
        ...
        if(vf->ready_state==INITSET){
          if(vf->current_serialno!=ogg2_page_serialno(&og)){
    ...
    */
    ...
    if(vf->ready_state!=INITSET){ 
      ...
      if(vf->ready_state<STREAMSET){
	if(vf->seekable){
	  long serialno=ogg2_page_serialno(&og);
	  ...
	  ogg2_stream_reset_serialno(vf->os,vf->current_serialno);
	  vf->ready_state=STREAMSET;
    ...
    if(og.header) { //only do this if there is a page ready !!!
    	ogg2_stream_pagein(vf->os,&og);
    }
  }
 cleanup:
...
static int _ov_open1(void *f,OggVorbis_File *vf,char *initial,
  ...
  /* perhaps some data was previously read into a buffer for testing
     ...
     non-seekable stream) */
  if(initial){
    unsigned char *buffer=ogg2_sync_bufferin(vf->oy,ibytes);
    memcpy(buffer,initial,ibytes);
    ogg2_sync_wrote(vf->oy,ibytes);
  }
  ...
  vf->os=ogg2_stream_create(-1); /* fill in the serialno later */
...
int ov_raw_seek(OggVorbis_File *vf,ogg_int64_t pos){
  ...
  /* don't yet clear out decoding machine (if it's initialized), in
     ...
     boundary */
  vf->pcm_offset=-1;
  ogg2_stream_reset_serialno(vf->os,
  ...
  */

  {
    ...
    work_os=ogg2_stream_create(vf->current_serialno); /* get the memory ready */
    while(1){
      if(vf->ready_state>=STREAMSET){
	/* snarf/scan a packet if we can */
	int result=ogg2_stream_packetout(work_os,&op);
	...
	if(result>0){
	  ...
	  if(vf->vi[vf->current_link].codec_setup){
	    ...
	    if(thisblock<0){
	      ogg2_stream_packetout(vf->os,NULL);
	      thisblock=0;
	    }else{
	...
	for(link=0;link<vf->links;link++)
	  if(vf->serialnos[link]==vf->current_serialno)break;
        ...
        vf->current_serialno=serialno;
	ogg2_stream_reset_serialno(vf->os,vf->current_serialno);
	ogg2_stream_reset_serialno(work_os,vf->current_serialno);
	vf->ready_state=STREAMSET;
      ...
      {
	ogg_page dup;
	ogg2_page_dup(&dup,&og);
	lastflag=ogg2_page_eos(&og);
	ogg2_stream_pagein(vf->os,&og);
	ogg2_stream_pagein(work_os,&dup);
      }
    }
  }
...
int ov_pcm_seek_page(OggVorbis_File *vf,ogg_int64_t pos){
  ...
  ogg_page og={0,0,0,0};
  ogg_packet op={0,0,0,0,0,0};
  vf->callbacks.print_func(vf->datasource, "ov_pcm_seek_page\n");
  ...
  /* new search algorithm by HB (Nicholas Vinen) */
  {
	...
	if(result<0){
	  ...
	  ogg_int64_t granulepos=ogg2_page_granulepos(&og);
	...
	vf->current_link=link;
	vf->current_serialno=ogg2_page_serialno(&og);
	vf->ready_state=STREAMSET;
	...
	vorbis_synthesis_restart(&vf->vd);
      ...
      while(1){
	result=ogg2_stream_packetpeek(vf->os,&op);
	if(result==0){
	  ...
	  while(1){
	    ...
	    if(result<0) goto seek_error;
            if(ogg2_page_serialno(&og)==vf->current_serialno &&
               (ogg2_page_granulepos(&og)>-1 ||
                !ogg2_page_continued(&og))){
	...
	if(op.granulepos!=-1){
	  ...
	  result=ogg2_stream_packetout(vf->os,NULL);
      }
    }
  }
...
int ov_pcm_seek(OggVorbis_File *vf,ogg_int64_t pos){
  ...
  vf->callbacks.print_func(vf->datasource, "ov_pcm_seek\n");
  ...
  while(1){
    ...
    int ret=ogg2_stream_packetpeek(vf->os,&op);
    if(ret>0){
      thisblock=vorbis_packet_blocksize(vf->vi+vf->current_link,&op);
      if(thisblock<0){
	ogg2_stream_packetout(vf->os,NULL);
	...
	for(i=0;i<vf->current_link;i++)
      ...
      if(ogg2_page_bos(&og))_decode_clear(vf);
	...
	int link;
        ...
        vf->ready_state=STREAMSET;
        vf->current_serialno=ogg2_page_serialno(&og);
        ogg2_stream_reset_serialno(vf->os,serialno);
        ret=_make_decode_ready(vf);
        if(ret){
	  ogg2_page_release(&og);
	  ogg2_packet_release(&op);
...
long ov_read(OggVorbis_File *vf,char *buffer,int bytes_req,int *bitstream){
  ...
  if(samples>0){
    ...
    vorbis_synthesis_read(&vf->vd,samples);
    vf->pcm_offset+=samples;
    if(bitstream)*bitstream=vf->current_link;   //bitstream pointing to iCodecActive
