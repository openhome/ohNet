...
std::vector<NetworkAdapter*>* OpenHome::Os::NetworkListAdapters(TBool aUseLoopback, const TChar* aCookie)
{
    ...
    while (ptr != NULL) {
        NetworkAdapter* iface = new NetworkAdapter(ptr->iAddress, ptr->iNetMask, ptr->iName, aCookie);
        ifs->push_back(iface);
        ptr = ptr->iNext;
    }
